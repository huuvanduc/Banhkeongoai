...
- Select a Vietnamese carrier/aggregator and a hosted QR/card provider. Implement their exact request/response mapping in `server/services/integrations.js`, then set the matching environment values.
...
Do not set `accepting_orders`, `shipping_enabled`, or `payment_enabled` merely to remove a blocker. Enable them only after the actual integration has passed its sandbox and security tests.
